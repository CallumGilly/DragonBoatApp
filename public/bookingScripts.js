    ...
    const options = {
      ...
      fetch(window.location.href, options)
        .then(response => {
          ...
          if (data.weatherPos == -1) {
            document.getElementById("WeatherBox").hidden = true;
          } else {
            document.getElementById("WeatherBox").hidden = false;
            document.getElementById("minTemp").innerHTML = "Min Temp: " +data.weatherData.daily.temperature_2m_min[data.weatherPos] + "°C"
            document.getElementById("maxTemp").innerHTML = "Max Temp: " +data.weatherData.daily.temperature_2m_max[data.weatherPos] + "°C"
            document.getElementById("maxWindspeed").innerHTML = "Max Windspeed: " +data.weatherData.daily.windspeed_10m_max[data.weatherPos] + "kn"
            document.getElementById("maxGusts").innerHTML = "Max Gusts: " +data.weatherData.daily.windgusts_10m_max[data.weatherPos] + "kn"
            document.getElementById("Rainfall").innerHTML = "Rainfall: " +data.weatherData.daily.precipitation_sum[data.weatherPos] + "mm"
          }
